...
contract FatherModifier {
    ...
    function changeOwner(address newOwner) public virtual onlyOwner {  // 被onlyOwner修饰器 修饰的 函数 
        owner = newOwner;
    }
}
...
contract ChildModifier is FatherModifier {
    ...
    function someFunction() public onlyOwner {   // 我们可以直接使用父合约中的onlyOwner修饰器
      
        // 只有所有者才能执行这里的代码
    }

    modifier limitValue(uint256 _value) {     // 定义一个新的修饰器：限制输入值在0到10之间
    ...
    function doSomethingWithValue(uint256 value) public limitValue(value) {  // 一个被新定义的limitValue修饰器修饰的函数
        // 执行一些操作，但输入值必须在0到10之间
    }

    // 重写 父合约中的 changeOwner函数。注意：由于 changeOwner函数 在父合约中 已经被 onlyOwner修饰器 修饰，所以这里不需要再次修饰
